...
3. 请通过浏览器或 Maven 仓库，下载以下 **4 个核心 Jar 包**，并放入 `flink-jars` 文件夹中（这些包将挂载给 Docker，让集群认识 Kafka、Paimon、S3 和 Hadoop 文件系统相关类）：
    ...
    * `hadoop-hdfs-client-3.3.4.jar`
    ...
    若使用命令行下载，可在 `bigdata-lab` 目录下执行：
    ```bash
    curl -fL -o flink-jars/hadoop-hdfs-client-3.3.4.jar \
      https://repo.maven.apache.org/maven2/org/apache/hadoop/hadoop-hdfs-client/3.3.4/hadoop-hdfs-client-3.3.4.jar
    ```
    如果是在服务启动后才补充 Jar 包，请执行 `docker compose restart dinky jobmanager taskmanager` 让 Dinky 与 Flink 重新加载依赖。
